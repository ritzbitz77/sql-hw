...
SELECT models.name, brand_name, founded FROM models LEFT JOIN brands 
ON (models.brand_name=brands.name) WHERE models.year=1960;
...
SELECT b.name, b.founded, m.name FROM brands b
LEFT JOIN models m ON b.name= m.brand_name WHERE b.discontinued IS null;
...
SELECT m.name, m.brand_name, b.founded FROM MODELS m JOIN brands b ON b.name = m.brand_name;
